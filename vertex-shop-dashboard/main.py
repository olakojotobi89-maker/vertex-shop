...
class VertexShopApp(ctk.CTk):
    ...
    def __init__(self):
        print("[Startup] Vertex Shop window initializing...")
        super().__init__()
        ...
        if name != "add_product":
            if name in ("dashboard", "products", "orders", "customers", "categories"):
                try:
                    ...
                    print(f"[ERROR] Failed to refresh view '{name}':")
                    traceback.print_exc()
                    self.toast_global(f"Failed to refresh {name}: {exc}", color=settings.COLORS["danger"])
    ...
    def open_edit_product(self, product_id):
        from database.database import db  # Import here as it is only used for this action
    ...
    def _refresh_current(self):
        if self.current_view and self.current_view in self.views:
            try:
                ...
                # Log refresh errors clearly instead of silently swallowing them.
                print(f"[MAIN] Error refreshing view '{self.current_view}': {exc}")
                traceback.print_exc()
...
def main():
    try:
        app = VertexShopApp()
        app.mainloop()
        print("[Shutdown] mainloop() exited normally.")
    except Exception:
        traceback.print_exc()
        input("\nVertex Shop crashed on startup. Press Enter to close...")
        raise
